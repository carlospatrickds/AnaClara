...
# Criando abas
aba1, aba2 = st.tabs(["🧮 Calculadora Trabalhista", "📚 Como Usar"])
...
with aba2:
    st.title("📚 Guia de Uso - Calculadora Trabalhista")
    
    st.header("🎯 Funcionalidades Principais")
    
    st.subheader("1. **Cálculos Realizados:**")
    st.write("""
    - **Salário mínimo** por competência (2020-2026)
    - **Periculosidade** (30% do salário base)
    - **Insalubridade** (10%, 20% ou 40% do salário mínimo)
    - **Horas noturnas** (adicional de 20%)
    - **Horas extras** (50%, 100% e percentual personalizado)
    """)
    
    st.subheader("2. **Fluxo do Sistema:**")
    ...
    st.subheader("3. **Recursos Técnicos:**")
    st.write("""
    - Validação de formato da competência
    ...
    st.write("""
    1. **Competência**: Use o formato MM/AAAA (ex: 12/2024)
    2. **Divisor de Jornada**: Normalmente 220 horas para mensalista
    3. **Horas Extras Personalizadas**: Para percentuais diferentes de 50% ou 100%
    4. **Insalubridade**: Baseada no salário mínimo vigente
    5. **Periculosidade**: Calculada sobre o salário base
    """)
    
    st.header("⚖️ Base Legal")
    st.write("""
    Calculadora baseada na CLT (Consolidação das Leis do Trabalho):
    - Art. 193 - Periculosidade
    - Art. 189 - Insalubridade  
    - Art. 73 - Horas Noturnas
    - Art. 59 - Horas Extras
    """)
    
    st.info("""
    **Resumo**: Esta é uma calculadora trabalhista completa que ajuda a calcular 
    todos os adicionais que um funcionário tem direito com base na legislação 
    brasileira, mostrando o passo a passo de todos os cálculos.
    """)
