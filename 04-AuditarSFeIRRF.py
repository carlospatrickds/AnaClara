    ...
    if opcao_entrada == "📁 Upload de CSV":
        ...
        with col_sheet1:
            sheets_url = st.text_input(
                ...
                help="Cole a URL completa da planilha do Google Sheets. A planilha deve ser pública.",
                key="sheets_url"
            )
                ...
                try:
                    ...
                    if len(df.columns) >= 3:
                        ...
                        if len(df.columns) > 3:
                         ...
                         st.warning("⚠️ Planilha deve ter no mínimo as colunas: Nome, Salario_Bruto, Dependentes.")
        ...
        num_funcionarios = st.number_input(
            ...
            min_value=1,
            max_value=50,
            value=max(3, len(st.session_state.dados_manuais)) if st.session_state.dados_manuais else 3,
            step=1,
        ...
        if len(st.session_state.dados_manuais) < num_funcionarios:
            ...
            novos_dados = [{
                'Nome': f"Funcionário {i+1+len(st.session_state.dados_manuais)}", 
    ...
    if st.session_state.df_resultado is not None:
        ...
        with col_csv:
            ...
            st.download_button(
                ...
                mime="text/csv",
                help="Baixe os resultados em CSV (separador ponto e vírgula, decimal vírgula)"
            )
