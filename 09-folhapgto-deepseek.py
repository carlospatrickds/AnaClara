...
def gerar_pdf_auditoria_completa(df_resultado, uploaded_filename, total_salario_familia, total_inss, total_irrf, folha_liquida_total, tabelas, observacoes=""):
    ...
    pdf.set_font('Arial', '', 9)
    metodologia = [
        '1. SALÁRIO FAMÍLIA: Pago para salários menores ou iguais ao limite, no valor por dependente',
        '2. INSS: Cálculo progressivo por faixas conforme tabela (Aliquota Efetiva)',
        '3. IRRF: Base de cálculo = Salário Bruto - Dependentes × Dedução - INSS - Outros Descontos',
        '4. Aplicadas alíquotas progressivas conforme tabela IRRF',
...
with tab1:
    ...
    # --- CAIXA DE OBSERVAÇÕES PARA CÁLCULO INDIVIDUAL ---
    st.subheader("📝 Observações para o Relatório")
    observacoes_individual = st.text_area(
        "Adicione observações que aparecerão no relatório PDF:",
        placeholder="Ex: Funcionário em período de experiência, análise especial requerida, observações sobre os cálculos...",
        height=100,
        key="observacoes_individual"
    )
    
    if st.button("Calcular", type="primary"):
        # Obter tabelas conforme ano da competência
        ano_competencia = competencia.year
        tabelas = obter_tabelas_por_ano(ano_competencia)
        
        inss_valor = calcular_inss(salario, tabelas['TABELA_INSS'], tabelas['TETO_INSS'])
        sal_familia = calcular_salario_familia(salario, dependentes, tabelas['SALARIO_FAMILIA_LIMITE'], tabelas['VALOR_POR_DEPENDENTE'])
        irrf_valor = calcular_irrf(salario, dependentes, inss_valor, tabelas['DESCONTO_DEPENDENTE_IR'], tabelas['TABELA_IRRF'], outros_descontos)
        ...
        base_irrf = salario - (dependentes * tabelas['DESCONTO_DEPENDENTE_IR']) - inss_valor - outros_descontos
        
        st.success(f"Cálculos realizados com sucesso! (Ano de referência: {tabelas['ANO']})")
        ...
        try:
            pdf = gerar_pdf_individual(dados_pdf, tabelas)
...
with tab2:
    ...
    **📊 Opções de Entrada de Dados:**
    ...
    # Competência para auditoria em lote
    st.subheader("📅 Competência para Auditoria")
    competencia_lote = st.date_input(
        "Competência Analisada (Lote)", 
        value=get_br_datetime_now().date().replace(day=1),
        format="DD/MM/YYYY",
        key="competencia_lote"
    )
    ...
    # --- CAIXA DE OBSERVAÇÕES PARA AUDITORIA EM LOTE ---
    st.subheader("📝 Observações para o Relatório em Lote")
    observacoes_lote = st.text_area(
        "Adicione observações que aparecerão no relatório PDF:",
        placeholder="Ex: Auditoria realizada para verificação de conformidade, observações gerais sobre a folha, comentários sobre funcionários específicos...",
        height=100,
        key="observacoes_lote"
    )
        ...
        try:
            ...
            if st.button("🚀 Processar Auditoria Completa", type="primary", key="processar_auditoria"):
                with st.spinner("Processando auditoria..."):
                    # Obter tabelas conforme ano da competência
                    ano_competencia = competencia_lote.year
                    tabelas = obter_tabelas_por_ano(ano_competencia)
                    ...
                    for _, row in df.iterrows():
                        ...
                        inss = calcular_inss(salario_bruto, tabelas['TABELA_INSS'], tabelas['TETO_INSS'])
                        sal_familia = calcular_salario_familia(salario_bruto, dependentes, tabelas['SALARIO_FAMILIA_LIMITE'], tabelas['VALOR_POR_DEPENDENTE'])
                        irrf = calcular_irrf(salario_bruto, dependentes, inss, tabelas['DESCONTO_DEPENDENTE_IR'], tabelas['TABELA_IRRF'], outros_desc)
                    ...
                    st.session_state.tabelas_utilizadas = tabelas
                    st.success(f"🎉 Auditoria concluída! (Ano de referência: {tabelas['ANO']})")
                    st.rerun()
    ...
    if st.session_state.df_resultado is not None:
        ...
        tabelas = st.session_state.tabelas_utilizadas
        
        st.info(f"📊 **Dados processados de:** {st.session_state.uploaded_filename} | **Ano de referência:** {tabelas['ANO']}")
        
        if st.button("🗑️ Limpar Resultados", type="secondary", key="limpar_resultados"):
            st.session_state.df_resultado = None
            st.session_state.uploaded_filename = None
            st.session_state.dados_manuais = []
            st.session_state.tabelas_utilizadas = None
...
with tab3:
    ...
    **🔍 Sistema de Detecção Automática:**
    O sistema detecta automaticamente o ano da competência e aplica as tabelas corretas:
    - **2024:** Tabelas atualizadas conforme MP 1.206/2024
    - **2025:** Tabelas projetadas (valores estimados)
    """)
    ...
    with col_info1:
        ...
        # Mostrar valores para ambos os anos
        st.write("**2024:**")
        st.write(f"""
        - Limite Salário Família: {formatar_moeda(1819.26)}
        - Valor por Dependente: {formatar_moeda(62.04)}
        - Dedução IR por Dependente: {formatar_moeda(189.59)}
        """)
        
        st.write("**2025:**")
        st.write(f"""
        - Limite Salário Família: {formatar_moeda(1906.04)}
        - Valor por Dependente: {formatar_moeda(65.00)}
        - Dedução IR por Dependente: {formatar_moeda(189.59)}
        """)
        
        st.subheader("📋 Como Calcular - Salário Família")
        st.code("""
Se Salário Bruto <= Limite:
    Salário Família = Nº Dependentes × Valor por Dependente
Senão:
    ...
    with col_info2:
        ...
        tabela_inss_2024_df = pd.DataFrame([
        ...
        st.dataframe(tabela_inss_2024_df, use_container_width=True, hide_index=True)
        st.caption(f"**Teto máximo do INSS 2024:** {formatar_moeda(908.85)}")
        
        st.subheader("📋 Como Calcular - INSS")
        st.code("""
Fórmula Progressiva (Aliquota Efetiva):
    Soma dos valores calculados sobre cada faixa
    (Ex: R$ 1.412,00 * 7,5% + (R$ 2.666,68 - R$ 1.412,00) * 9% + ...)
        """)

    st.subheader("📈 Tabela IRRF 2024 (MP 1.206/2024)")
    tabela_irrf_2024_df = pd.DataFrame([
    ...
    st.dataframe(tabela_irrf_2024_df, use_container_width=True, hide_index=True)
    
    st.subheader("📋 Como Calcular - IRRF")
    st.code("""
Base de Cálculo = Salário Bruto - (Dependentes × Dedução) - INSS - Outros Descontos
IRRF = (Base de Cálculo × Alíquota da Faixa) - Parcela a Deduzir da Faixa
    """)

    st.subheader("📝 Legislação de Referência 2024")
    st.write("""
    - **Salário Família:** Lei 8.213/1991
    - **INSS:** Lei 8.212/1991 e Portaria Interministerial MPS/MF nº 2/2024
    - **IRRF:** Lei 7.713/1988 e MEDIDA PROVISÓRIA Nº 1.206/2024
...
- Salário Família
- INSS (Tabelas atualizadas)
- IRRF (Conforme MP 1.206/2024)
...
with col_rodape2:
    st.caption("🏛 Legislação Atualizada - MP 1.206/2024")
