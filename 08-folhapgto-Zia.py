...
def gerar_pdf_individual(dados, obs):
    """Gera PDF profissional para cálculo individual (MODIFICADO para incluir OBS e referências obrigatórias)"""
    pdf = FPDF()
    pdf.add_page()
    ...
    if obs:
        ...
        pdf.multi_cell(0, 6, "Nenhuma observação registrada pelo analista.")
    pdf.ln(5)
    ...
    # Legislação e Metodologia (SEÇÃO OBRIGATÓRIA E AMPLIADA)
    pdf.set_font('Arial', 'B', 12)
    ...
    # Detalhes sobre a MP 1.294/2025 (se aplicável)
    if dados["irrf_periodo"] == "01/05/2025 em diante (MP 1.294)":
        pdf.set_font('Arial', 'B', 10)
        pdf.cell(0, 6, 'DETALHES DA MP 1.294/2025', 0, 1)
        pdf.set_font('Arial', '', 9)
        mp_detalhes = [
            '- Medida Provisória nº 1.294, de 12 de abril de 2025',
            '- Altera a tabela do Imposto de Renda da Pessoa Física',
            '- Ajusta as faixas de isenção e alíquotas do IRRF',
            '- Nova faixa de isenção: R$ 2.428,80',
            '- Redução da carga tributária para trabalhadores de menor renda',
            '- Vigência a partir de 1º de maio de 2025'
        ]
        for item in mp_detalhes:
            pdf.multi_cell(0, 5, item)
            pdf.ln(1)
...
def gerar_pdf_auditoria_completa(df_resultado, uploaded_filename, total_salario_familia, total_inss, total_irrf, folha_liquida_total, obs_lote):
    ...
    # --- OBSERVAÇÕES EM LOTE (OBRIGATÓRIO) ---
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 10, 'OBSERVAÇÕES DO ANALISTA', 0, 1)
    pdf.set_font('Arial', '', 10)
    if obs_lote:
    ...
    # Detalhes sobre a MP 1.294/2025 (se aplicável)
    if irrf_periodo == "01/05/2025 em diante (MP 1.294)":
        pdf.set_font('Arial', 'B', 10)
        pdf.cell(0, 6, 'DETALHES DA MP 1.294/2025', 0, 1)
        pdf.set_font('Arial', '', 9)
        mp_detalhes = [
...
with tab1:
    ...
    observacao_individual = st.text_area(
        "Observação (Será incluída no PDF)",
        value="",
        height=100,
        help="Esta observação será incluída no relatório PDF como parte obrigatória da auditoria."
    )
...
with tab2:
    ...
    **📊 Opções de Entrada de Dados:**
    ...
    # Campo de observação em lote (OBRIGATÓRIO)
    observacao_lote = st.text_area(
        "Observação Geral (Será incluída no PDF Completo)",
...
with tab3:
    ...
    **Medida Provisória 1.294/2025:**
    - Publicada em 12 de abril de 2025
    - Altera a tabela do Imposto de Renda da Pessoa Física
    - Nova faixa de isenção: R$ 2.428,80
    - Vigência a partir de 1º de maio de 2025
    - Redução da carga tributária para trabalhadores de menor renda
...
IRRF = (Base de Cálculo × Alíquota da Faixa) - Parcela a Deduzir da Faixa
    ...
    - **IRRF (Atualizações):** Lei 7.713/1988, Lei nº 14.663/2023, e MP nº 1.294/2025
    """)
